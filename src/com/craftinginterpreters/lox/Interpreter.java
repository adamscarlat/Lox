...
class Interpreter implements Expr.Visitor<Object>, Stmt.Visitor<Void> {
    ...
    public Void visitClassStmt(Stmt.Class stmt) {
        ...
        List<LoxClass> superclassesLox = new ArrayList<>();
        if (stmt.superclasses != null && stmt.superclasses.size() > 0) {
            for (Expr.Variable superclassCurr : stmt.superclasses) {
                Object superclass = evaluate(superclassCurr);
                if (!(superclass instanceof LoxClass)) {
                    throw new RuntimeError(superclassCurr.name, "Superclass must be a class.");
                }

                environment = new Environment(environment);
                environment.define("super", superclass);

                superclassesLox.add((LoxClass)superclass);
            }
...
//        }
        ...
        LoxClass klass = new LoxClass(stmt.name.lexeme, superclassesLox, methods);
    ...
    public Object visitSuperExpr(Expr.Super expr) {
        ...
        List<Object> superclassObjects = environment.getAll("super");

        LoxFunction method = null;
        LoxInstance object = null;
        for (Object superclassObject : superclassObjects) {
            LoxClass superclass = (LoxClass) superclassObject;
            // get the calling object - its environment will always be one level below
            // the superclass' environment
            object = (LoxInstance)environment.getAt(distance - 1, "this");
